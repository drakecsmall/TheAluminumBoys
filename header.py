...
'''
	questions = ',\n'.join([questionText.replace("_QUESTION_",i.question).replace("_ANSWERS_",'"' + '", "'.join(i.answers.keys())+'"').replace("_CORRECT_",''.join([j if i.answers[j] == 1 else '' for j in i.answers.keys()])).replace("_TEXT_",i.answertext) for i in q.questions])
	quizjs = quizjs.replace("_QUESTIONS_",questions)
	return '''	<!doctype html>
			<html lang="en">
  				<head>
    					<meta charset="utf-8">
    					...
    					{}
                                        <title>Electric City Data</title>
  				        {}
                                </head>
               '''.format(quizjs, style.BuildStyle())
